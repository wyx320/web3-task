...
describe("MetaNodeStake (Hardhat JS tests)", function () {
  ...
  it("AddPool: 添加原生代币迟并检查初始状态", async function () {
    await addNativePool();
    const pool = await metaNodeStake.stakePools(0);
    ...
    expect(pool.withDrawLockedBlocks).to.equal(100n);
  ...
  it("massUpdatePools: 在区块推进后更新 lastRewardBlock", async function () {
    await addNativePool();
  ...
  it("SetPoolWeight: 修改池权重并检查 totalWeight", async function () {
    await addNativePool();
    let preTotalWeight = await metaNodeStake.totalWeight;

    await metaNodeStake.updatePoolWeight(0, 200, false);
    let pool = await metaNodeStake.pools(0);
    let totalWeight = await metaNodeStake.totalWeight;

    expect(pool.poolWeight).to.equal(200n);
    expect(totalWeight).to.equal(preTotalWeight.add(100n)); // 之前是 100，现在变成 200，所以增加了 100
  });
  ...
  it("Deposit native currency: 存入原生代币并检查用户与质押池状态", async function () {
    ...
    expect(pool.stTotalAmount).to.equal(preStTotalAmount + 100n);
    expect(user.stAmount).to.equal(preStAmount + 100n);
    expect(user.finishedMetaNode).to(preFinishedMetaNode);
    ...
    await metaNode.withdraw(0);
  });

  it("Unstake: 解押后检查用户与质押池状态", async function () {
    await addNativePool();
    await metaNodeStake.depositETH({ value: parseEther("1") });

    // 推进区块高度
    await mineToBlock(1000);
    await metaNodeStake.unstake(0, parseEther("0.3"));

    // 检查用户状态
    const user = await metaNodeStake.users(0, owner.address);
    expect(user.stAmount).to.equal(parseEther("0.7"));
    ...
    expect(pool.stTokenAmount).to.equal(parseEther("0.7"));
  });
  ...
  it("Withdraw: 解质押后提取原生代币", async function () {
    ...
    expect(contractBalance).to.be.lt(BigInt(0.7));
    // 因为用户支付了 gas，这里用 大于等于 不严谨；为稳妥检查差值正向
    expect(userBalance).to.be.gt((preUserBalance)-parseEther("0.01"));
  });
  ...
  it("Claim after deposit: 存入后领取奖励", async function () {
    ...
    : metaNodeStake.address;
    await metaNode.transfer(stakeAddress, parseEther("1000"));

    const preUserBalance = await ethers.provider.getBalance(owner.address);
    const tx = await metaNodeStake.claim(0);
    await tx.wait();
    const userBalance = await ethers.provider.getBalance(owner.address);
  ...
  it("Claim after withdraw: 提取后领取奖励", async function () {
    await addNativePool();
    await metaNodeStake.depositETH({ value: parseEther("1") });
    await metaNodeStake.unstake(0, parseEther("1"));
